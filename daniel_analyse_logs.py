...
# load ~/mujoco{whichact}_qlog.csv
with open(os.path.expanduser(f'~/gym{whichact}_qlog.csv'), 'r') as f:
    lines = f.readlines()
    lines = [line.strip().split(',') for line in lines]
    lines = [[float(x) for x in line] for line in lines]
    mujoco_one = np.array(lines)

# load ~/wasm{whichact}_qlog.csv
with open(os.path.expanduser(f'~/wasm{whichact}_qlog.csv'), 'r') as f:
    lines = f.readlines()
    lines = [line.strip().split(',') for line in lines]
...
for i in range(8):
    axs[0].plot(mujoco_one[:, i], label=f'mujoco{whichact}_{i}')
    axs[1].plot(wasm_one[:, i], label=f'wasm{whichact}_{i}')
axs[0].legend()
axs[1].legend()
...
for i in range(8, 8+34, 2):
    axs[0].plot(mujoco_one[:, i], label=f'mujoco{whichact}_{joint_name_list[idx]}')
    axs[1].plot(wasm_one[:, i], label=f'wasm{whichact}_{joint_name_list[idx]}')
    axs[2].plot(mujoco_one[:, i] - wasm_one[:, i], label=f'diff_{joint_name_list[idx]}')
    idx += 1
axs[0].legend()
axs[1].legend()
axs[2].legend()
